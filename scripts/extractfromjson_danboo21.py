...
for i in range(12):
    ...
    ##Open the file
    json_file_path = json_file_name ##Name of the JSON file to use, converted into parser arg
    ...
    for json_str in json_list:
        ...
        if os.path.exists(file_path):
            ...
            if str(baru) == "False":
                print(img_id)
                print(str(args.imagespath) + "/0" + img_id_last3 + "/" + img_id + "." + img_ext)
                print("Entry rating' is not in acceptedRatings, skipping entry. WTF??????????###########################################################################################################################################################################################################################################")
                continue
            elif str(baru) == "True":
            ...
            to_write = FinalTagStringCharacter + FinalTagStringCopyright + tag_separator + FinalTagStringArtist + tag_separator + FinalTagStringRating + tag_separator + FinalTagStringGeneral + tag_separator
